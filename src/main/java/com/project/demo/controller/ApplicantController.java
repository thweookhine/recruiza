...
public class ApplicantController {
	...
	public ModelAndView addApplicant(@RequestParam("jobPostId") Long id, ModelMap model, RedirectAttributes ra,
			HttpSession session) {
	...
	public ModelAndView applicantProcess(@PathVariable("pageNumber") int currentPage,
			@PathVariable("keyword") String keyword, @Param("sortField") String sortField,
			@Param("sortDir") String sortDir, @Param("searchKey") String searchKey, ModelMap model) {
	...
	public ModelAndView nextApplicantProcess(@RequestParam("id") Long applicantId, ModelMap model,
		...
		ApplicantBean applicantBean = ApplicantBean.builder().applicantId(applicant.getApplicantId())
				.applicantCode(applicant.getApplicantCode()).applicantName(applicant.getApplicantName())
				.applicantEmail(applicant.getApplicantEmail()).applicantMobile(applicant.getApplicantMobile())
				.address(applicant.getAddress()).link(applicant.getLink()).comment(applicant.getComment())
				.currentState(applicant.getCurrentState()).applicantStatus(applicant.getApplicantStatus())
				.jobPositionBean(applicant.getJobPosition().getPositionName())
				.jobPostBean(applicant.getJobPost().getPostName()).applyTime(applicant.getApplyTime().toString())
				.build();
		return new ModelAndView("applicantChange", "bean", applicantBean);
	}

	@PostMapping(value = "/applicantStatusChange")
	public ModelAndView applicantStatusChange(@ModelAttribute("bean") ApplicantBean bean, HttpServletRequest request,
			ModelMap model) {
		String statusString = bean.getApplicantStatus();

		Applicant applicant = service.changeApplicantStatus(bean, request.getParameter("checkStatus"),
				request.getParameter("getStatus"), request.getParameter("comment"));
	...
	public ModelAndView saveApplicant(HttpServletRequest request, RedirectAttributes ra, ModelMap model,@RequestParam("file")MultipartFile file)
			throws IOException {
		...
		if (service.findApplicantByEmailAndJobPost(request.getParameter("email"), jobPost.getPostId()) != null) {
			model.addAttribute("message", "User email is has been !");
		} else {

			Applicant applicant = Applicant.builder().applicantName(request.getParameter("name"))
					.applicantEmail(request.getParameter("email")).applicantMobile(request.getParameter("mobile"))
					.address(request.getParameter("address")).link(request.getParameter("link"))
					.comment(request.getParameter("comment")).applyTime(Timestamp.valueOf(LocalDateTime.now()))
					.applicantStatus("None").currentState("PENDING").jobPosition(jobPost.getJobPosition())
					.jobPost(jobPost).file(file.getBytes())
					.source(request.getParameter("Source")).build();
	...
	public ModelAndView saveApplicants(JobPost jpost, HttpServletRequest request, RedirectAttributes ra,
		...
		if (service.findApplicantByEmailAndJobPost(request.getParameter("email"), jobPost.getPostId()) != null) {
			...
			Applicant applicant = Applicant.builder().applicantName(request.getParameter("name"))
					...
					.comment(request.getParameter("comment")).applyTime(Timestamp.valueOf(LocalDateTime.now()))
					.applicantStatus("None").currentState("PENDING").jobPosition(jobPost.getJobPosition())
					.jobPost(jobPost).file(file.getBytes()).source("Direct Recruitement").build();
	...
	public ModelAndView editApplicant(@RequestParam("id") Long id, ModelMap model) {
		...
		ApplicantBean bean = ApplicantBean.builder().applicantId(applicant.getApplicantId())
				.applicantName(applicant.getApplicantName()).applicantEmail(applicant.getApplicantEmail())
				.applicantMobile(applicant.getApplicantMobile()).address(applicant.getAddress())
				.link(applicant.getLink()).comment(applicant.getComment())
				.jobPostBean(String.valueOf(applicant.getJobPost().getPostId()))
				.jobPositionBean(String.valueOf(applicant.getJobPosition().getPositionId())).build();
	...
	@GetMapping(value = "/viewFile/{id}", produces = { MediaType.APPLICATION_PDF_VALUE })
